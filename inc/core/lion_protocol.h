#ifndef _LION_PROTOCOL_H_
#define _LION_PROTOCOL_H_
...
public:
    ...
    bool     verify_crc8_check_sum(uint8_t *, uint16_t);
    ...
    // unpack method
    void serial_msg_unpack_handler(const uint8_t msg);

    // pack method
    int  serial_msg_pack_handler(uint16_t data_length, 
                                 uint8_t  seq, 
                                 uint16_t cmd_id, 
                                 const void * data);

    // read bytes from readable serial
    void read();
    // send bytes to serial
    bool send(const void* data, uint16_t cmd_id, int len);
...
private:
    ...
    // data
    std::mutex tx_frame_mutex;
    std::mutex rx_frame_mutex;

    robot_rc_t   rc;
    robot_pose_t pose;

    // transmit
    uint8_t*    serial_tx_buffer;
    uint16_t    serial_tx_msg_length;
    uint8_t     serial_tx_seq;

    // receive
    uint8_t*    serial_rx_buffer;
    uint8_t*    serial_rx_byte;
    uint16_t    serial_rx_msg_length;
    uint16_t    data_segment_length;
    uint8_t     serial_rx_buffer_index;
    bool        frame_header_available;
    bool        frame_total_available;    

    // CRC8生成多项式: G(x) = x8 + x5 + x4 + 1
